...
# ПРАВИЛА ВЫЗОВА ДЕЙСТВИЙ
Если для ответа клиенту тебе нужно выполнить одно из вышеуказанных действий, твой ответ ДОЛЖЕН начинаться со специального блока кода. Этот блок будет выполнен системой.

**СТРОГИЙ ФОРМАТ БЛОКА ДЕЙСТВИЙ:**
```json
{{
  "tool_calls": [
    {{
...
Если для выполнения задачи тебе нужно вызвать "исполнительный" инструмент, помести JSON с вызовом в блок ```json ... ``` в НАЧАЛЕ своего ответа, а затем напиши текст для клиента.
