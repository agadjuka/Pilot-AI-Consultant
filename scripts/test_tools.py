...
async def main():
    ...
    try:
        ...
        print("✅ Все зависимости инициализированы успешно")
        print()
...
async def test_get_masters_for_service(tool_service):
    ...
    try:
        ...
        print(f"✅ Результат для 'Мужская стрижка': {masters}")
    except Exception as e:
        print(f"❌ Ошибка для 'Мужская стрижка': {str(e)}")
    
    # Тест с несуществующей услугой
    try:
        masters_fail = tool_service.get_masters_for_service("Несуществующая услуга")
        print(f"✅ Результат для 'Несуществующая услуга': {masters_fail}")
    except Exception as e:
        print(f"❌ Ошибка для 'Несуществующая услуга': {str(e)}")
    print()
...
async def test_get_available_slots(tool_service):
    ...
    try:
        ...
        print(f"✅ Результат для 'Мужская стрижка' на 2025-10-25: {slots}")
    except Exception as e:
        print(f"❌ Ошибка для 'Мужская стрижка' на 2025-10-25: {str(e)}")
    ...
    try:
        ...
        print(f"✅ Результат для 'Несуществующая услуга' на 2025-10-25: {slots_fail}")
    except Exception as e:
        print(f"❌ Ошибка для 'Несуществующая услуга' на 2025-10-25: {str(e)}")
    ...
    try:
        ...
        print(f"✅ Результат для 'Мужская стрижка' на 2025-10-26: {slots_no_slots}")
    except Exception as e:
        print(f"❌ Ошибка для 'Мужская стрижка' на 2025-10-26: {str(e)}")
    print()
...
async def test_appointment_workflow(tool_service, client_repository):
    ...
    try:
        ...
        print(f"✅ Результат создания записи: {new_appointment}")
        
        # Шаг 2: Просмотр записей
        print("\nШаг 2: Просмотр записей...")
        my_appointments = tool_service.get_my_appointments(user_telegram_id=user_id)
        print(f"✅ Результат просмотра записей: {my_appointments}")
        ...
        if my_appointments and len(my_appointments) > 0:
            ...
            if my_appointments_after_cancel and len(my_appointments_after_cancel) > 0:
                ...
                print(f"✅ Результат переноса записи: {reschedule_result}")
            else:
                print("\nШаг 5: Нет записей для переноса")
        else:
            print("\n⚠️ Записи не найдены, пропускаем тесты отмены и переноса")
            
    except Exception as e:
        print(f"❌ Ошибка в цикле работы с записями: {str(e)}")
        import traceback
        traceback.print_exc()
    print()
...
async def test_call_manager(tool_service):
    ...
    try:
        ...
        print(f"✅ Результат вызова менеджера: {manager_result}")
    except Exception as e:
        print(f"❌ Ошибка вызова менеджера: {str(e)}")
    print()
